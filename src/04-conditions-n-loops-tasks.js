...
function doRectanglesOverlap(rect1, rect2) {
  const a = rect1; const
    b = rect2;
  a.right = rect1.left + rect1.width;
  a.bottom = rect1.top + rect1.height;
  b.right = rect2.left + rect2.width;
  b.bottom = rect2.top + rect2.height;
...
function findFirstSingleChar(str) {
  ...
  str.split('').forEach((elem) => {
    if (elem in lst) lst[elem] += 1;
    else lst[elem] = 1;
  });

  const d = Object.keys(lst);

  for (let i = 0; i < d.length; i += 1) {
    if (lst[d[i]] === 1) return d[i];
  }
...
function isCreditCardNumber(ccn) {
  const d = +String(ccn).slice(-1);
  let total = 0;
  let flag = true;
  const str = String(ccn).slice(0, -1);

  for (let i = -1; i >= -str.length; i -= 1) {
    const t = (+str.at(i)) * (flag ? 2 : 1);
    total += t > 9 ? t - 9 : t;
    flag = !flag;
  }
  let answer = 10 - (total % 10);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    else if (str[i] === obj[newStr.at(-2)]) {
      newStr = newStr.slice(0, -2);
    } else if (Object.keys(obj).includes(str[i]));
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    for (let j = 0; j < pathes[i].length; j += 1) {
      if (base[j] !== pathes[i][j]) base = base.slice(0, j).slice(0, base.lastIndexOf('/') + 1);
    }
  }
  return base;
}
